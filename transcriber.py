...
class AudioRecorder:
    ...
    async def record_until_silence(
        ...
        while (asyncio.get_event_loop().time() - start_time) < max_duration:
            ...
            is_silence = current_rms <= float(self.silence_threshold if not None else 0.0)
    ...
    async def transcribe_stream(
        ...
        with wave.open(str(temp_file), 'wb') as wf:
            ...
            async for chunk in audio_stream:
                if first_chunk:
                    wf.setnchannels(chunk.shape[1] if len(chunk.shape) > 1 else 1)
                    wf.setsampwidth(chunk.dtype.itemsize)
...
class TranscriptionService:
    ...
    async def transcribe_speech(
        ...
        audio_stream = recorder.record_until_silence(
